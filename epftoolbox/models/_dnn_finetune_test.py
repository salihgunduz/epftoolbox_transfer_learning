...
class DNNModel(object):
    ...
    def __init__(self, neurons, n_features, outputShape=24, dropout=0, batch_normalization=False, lr=None,
                 ...
                 experiment_id = '1', target='FR', source='BE', DNN_id='3'):
...
class DNN(object):
    ...
    def __init__(self, experiment_id, path_hyperparameter_folder=os.path.join('.', 'experimental_files'),
        ...
        if not os.path.exists(self.path_hyperparameter_folder):
            raise Exception(
                'Provided directory for hyperparameter file does not exist')
        ...
        self.hyperparameter_file_name = hyperparameter_file_name
        self.target = target
        self.source = source 
        self.DNN_id = DNN_id 
    ...
    def _read_best_hyperapameters(self):
        ...
        trials_file_name = self.hyperparameter_file_name
...
# =============================================================================
    ...
    def recalibrate(self, Xtrain, Ytrain, Xval, Yval):
        ...
        self.model = DNNModel(neurons=neurons, n_features=Xtrain.shape[-1],
                              ...
                              target=self.target, source=self.source, DNN_id=self.DNN_id
                              )
    ...
    def recalibrate_and_forecast_next_day(self, df, next_day_date):
        ...
        df_train = df_train.loc[next_day_date -
                                pd.Timedelta(hours=self.calibration_window * 364 * 24):]
...
# =============================================================================
        ...
        Xtrain, Xval, Xtest, Ytrain, Yval = \
            self._regularize_data(Xtrain=Xtrain, Xval=Xval,
                                  Xtest=Xtest, Ytrain=Ytrain, Yval=Yval)
...
def evaluate_finetune_in_test_dataset(experiment_id, path_datasets_folder=os.path.join('.', 'datasets'),
                                 path_hyperparameter_folder=os.path.join(
                                     '.', 'experimental_files'),
                                 path_recalibration_folder=os.path.join(
                                     '.', 'experimental_files'),
                                 hyperparameter_file_name = 'DNN_hyperparameters_nl2_datFR_YT2_SF_SF_CW4_1',
                                 target='FR', source='BE', DNN_id='3',
    ...
    forecast_file_path = os.path.join(
        path_recalibration_folder, forecast_file_name)

    # Defining empty forecast array and the real values to be predicted in a more friendly format
    forecast = pd.DataFrame(index=df_test.index[::24], columns=[
                            'h' + str(k) for k in range(24)])
    ...
    if not new_recalibration:
        ...
        if len(forecast_dates) == 0:
            ...
            print(
                '{} - sMAPE: {:.2f}%  |  MAE: {:.3f}'.format('Final metrics', smape, mae))

    else:
        forecast_dates = forecast.index

    model = DNN(experiment_id=experiment_id, path_hyperparameter_folder=path_hyperparameter_folder,
                hyperparameter_file_name = hyperparameter_file_name, target=target, source=source, DNN_id=DNN_id,
    ...
    for date in forecast_dates:
        ...
        data_available.loc[date:date +
                           pd.Timedelta(hours=23), 'Price'] = np.NaN

        # Recalibrating the model with the most up-to-date available data and making a prediction
        # for the next day
        Yp = model.recalibrate_and_forecast_next_day(
            df=data_available, next_day_date=date)
        ...
        mae = np.mean(
            MAE(forecast.loc[:date].values.squeeze(), real_values.loc[:date].values))
        smape = np.mean(
            sMAPE(forecast.loc[:date].values.squeeze(), real_values.loc[:date].values)) * 100

        # Pringint information
        print(
            '{} - sMAPE: {:.2f}%  |  MAE: {:.3f}'.format(str(date)[:10], smape, mae))
...
def _build_and_split_XYs(dfTrain, features, shuffle_train, n_exogenous_inputs, dfTest=None, percentage_val=0.25,
    ...
    if date_test is None:
        ...
        indexTest = dfTest.loc[date_test:date_test +
                               pd.Timedelta(hours=23)].index
    ...
    indexTrain = pd.DataFrame(index=predDatesTrain, columns=[
                              'h' + str(hour) for hour in range(24)])
    indexTest = pd.DataFrame(index=predDatesTest, columns=[
                             'h' + str(hour) for hour in range(24)])
    for hour in range(24):
        indexTrain.loc[:, 'h' +
                       str(hour)] = indexTrain.index + pd.Timedelta(hours=hour)
        indexTest.loc[:, 'h' + str(hour)] = indexTest.index + \
    ...
    for hour in range(24):
        ...
        for past_day in [1, 7]:
            ...
            for exog in range(1, n_exogenous_inputs + 1):
                if features['In: Exog-' + str(exog) + ' D-' + str(past_day)]:
                    Xtrain[:, indexFeatures] = dfTrain.loc[pastIndexTrain,
                                                           'Exogenous ' + str(exog)]
                    Xtest[:, indexFeatures] = dfTest.loc[pastIndexTest,
                                                         'Exogenous ' + str(exog)]
                    indexFeatures += 1
        ...
        for exog in range(1, n_exogenous_inputs + 1):
            ...
            if features['In: Exog-' + str(exog) + ' D']:
                ...
                Xtrain[:, indexFeatures] = dfTrain.loc[futureIndexTrain,
                                                       'Exogenous ' + str(exog)]
                Xtest[:, indexFeatures] = dfTest.loc[futureIndexTest,
                                                     'Exogenous ' + str(exog)]
                indexFeatures += 1
